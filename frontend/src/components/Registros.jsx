...
function Registros({ usuario }) {
  ...
  const cargarRegistros = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/registros?${params.toString()}`);
  ...
  const limpiarFiltros = () => {
    setFiltros({ area: '', inspector: esAdmin ? '' : usuario?.id, fechaInicio: '', fechaFin: '' });
    setPage(1);
      ...
      <div className="registros-container">
        ...
        <div className="filtros-bar">
          <div className="filtro-grupo">
            <label><MapPin size={10} /> Área</label>
            <select
              value={filtros.area}
            ...
            >
              ...
              {areas.map(a => (
                <option key={a.COD_AREA} value={a.COD_AREA}>{a.DESCRIPCION}</option>
              ))}
            </select>
          </div>
          ...
          {esAdmin && (
            <div className="filtro-grupo">
              <label><User size={10} /> Inspector</label>
              <select
                value={filtros.inspector}
                onChange={(e) => setFiltros(prev => ({ ...prev, inspector: e.target.value }))}
              >
                <option value="">Todos los inspectores</option>
                {inspectores.map(i => (
                  <option key={i.COD_USR || i.id} value={i.COD_USR || i.id}>{i.NOMBRE || i.nombre}</option>
                ))}
              </select>
            </div>
          )}
          ...
          <div className="filtro-grupo">
            <label><Calendar size={10} /> Desde</label>
            <input
              type="date"
          ...
          <div className="filtro-grupo">
            <label><Calendar size={10} /> Hasta</label>
            <input
              type="date"
        ...
        <div className="registros-tabla-section">
          ...
          {cargando ? (
            ...
            <div className="empty-state">
              ...
              <p>Aún no se han realizado inspecciones o no hay resultados para los filtros aplicados.</p>
            </div>
          ) : vistaMode === 'matriz' ? (
            cargandoMatriz ? (
              <div className="loader-container">
                ...
                <table className="matriz-table">
                  <thead>
                    <tr>
                      ...
                      {datosMatriz.map(reg => (
                        <th key={reg.id} style={{ minWidth: '160px', textAlign: 'center' }}>
                          ...
                          <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', marginTop: '8px' }}>
                            <button
                              ...
                              style={{ padding: '2px 8px', fontSize: '10px', backgroundColor: 'white', color: 'var(--color-primary)', border: '1px solid white' }}
                    ...
                    {(() => {
                      ...
                      return pregs.map((p, pIdx) => {
                        ...
                        const getCellClass = (r, regId) => {
                          ...
                          if (r.resp_varchar && r.resp_varchar.includes('Causas:')) {
                            cls += ' has-comment';
                          }
                        ...
                        const getCellContent = (r) => {
                          ...
                          if (r.tipo_campo === 'N') {
                            const val = r.resp_number !== null && r.resp_number !== undefined ? r.resp_number : '—';
                            if (r.resp_varchar && r.resp_varchar.includes('Causas:')) {
